...
function DMW.Classes.Unit:SectorPull(check)
  if GetRaidTargetIndex("player") then
    local pullSectorsNumber = 0
    for k, v in pairs(DMW.Friends.Units) do
      if Unlocked.GetRaidTargetIndex(v.GUID) then
        pullSectorsNumber = pullSectorsNumber + 1
      end
    end
    local sectorAngle = 2 * math.pi / pullSectorsNumber
    local angleToPlayer = GetAnglesBetweenPositions(DMW.Player.PosX, DMW.Player.PosY, DMW.Player.PosZ, self.PosX,
      self.PosY, self.PosZ)
    ...
    -- 7 = Red "X" Cross
    -- 8 = White Skull
    local sector = math.floor(angleToPlayer / sectorAngle) + 1
    if check then
      print(sector)
      return
    end
    return sector == GetRaidTargetIndex("player")
  else
    return true
  end
end
...
local function isCurrentlyTanking()
  ...
  local IsTanking = Player:IsTankingAoE(16) or
      (DMW.Player.Target and (DMW.Player.Target:IsTanking() or DMW.Player.Target.Dummy))
  return IsTanking
end

local function HealthPotUse()
  if Player.Combat and Setting("Health Potion/HealthStone HP%") > 0 and
      Setting("Health Potion/HealthStone HP%") > Player.HP then
...
local function LocalsBalance()
  if Buff.EclipseLunar:Exist() and NextEclipse ~= "Solar" then
    NextEclipse = "Solar"
...
local function LocalsFeral()
  if Talent.BloodTalons then
    ...
    if DMW.Time - Spell.Rake.LastCastTime <= 4 then
      BloodTalonsGenRake = true;
      BloodTalonsGenStack = BloodTalonsGenStack + 1
    end
    if DMW.Time - Spell.Shred.LastCastTime <= 4 then
      BloodTalonsGenShred = true;
      BloodTalonsGenStack = BloodTalonsGenStack + 1
    end
    if DMW.Time - Spell.Moonfire.LastCastTime <= 4 then
      BloodTalonsGenMoonfire = true;
      BloodTalonsGenStack = BloodTalonsGenStack + 1
    end
    if not Talent.BrutalSlash and DMW.Time - Spell.Swipe.LastCastTime <= 4 then
      BloodTalonsGenSwipe = true;
      BloodTalonsGenStack = BloodTalonsGenStack + 1
    end
    if DMW.Time - Spell.Thrash.LastCastTime <= 4 then
      BloodTalonsGenThrash = true;
      BloodTalonsGenStack = BloodTalonsGenStack + 1
    end
    if Talent.BrutalSlash and DMW.Time - Spell.BrutalSlash.LastCastTime <= 4 then
      BloodTalonsGenBrutalSlash = true;
      BloodTalonsGenStack = BloodTalonsGenStack + 1
    end
  end
  noAoE = aoeCheck()
...
local function RestorationWildGrowth()
  if not Player.Moving and Spell.WildGrowth:IsReady() and Setting("WildGrowth HP") > 0 and
      Setting("WildGrowth Count") > 0 and RestorationMoonkinCheck() then
    ...
    if Player.ComboPoints >= 5 then
      ...
      if Talent.BloodTalons then
        if not Buff.BloodTalons:Exist() then
          if BloodTalonsGenStack == 0 and
              (Player.Energy + 3.5 * GetPowerRegen() + 40 * num(Buff.Clearcasting:Exist())) <
              115 then
            -- print("pool", DMW.Time)
            return true
          end
...
local function GuardianRotation()
  ...
  if Spell.Mangle:IsReady() and Player.Rage < 90 and EnemyMeleeCount < 3 then
    for _, Unit in ipairs(EnemyMelee) do
  ...
  if Spell.Maul:IsReady() and EnemyMeleeCount < 3 then
    for _, Unit in ipairs(EnemyMelee) do
...
function Druid.Rotation()
  ...
  if foundVendor and Player:GetFreeBagSlots() < 140 then
    if not MerchantFrame:IsVisible() then
      Unlocked.Interact(foundVendor.Pointer)
    else
      if DMW.Time - macroSellUse > 0.5 then
        if MerchantRepairAllButton:IsShown() and MerchantRepairAllButton:IsEnabled() then
          MerchantRepairAllButton:Click()
        end
        if MerchantSellAllJunkButton:IsShown() and MerchantSellAllJunkButton:IsEnabled() then
          MerchantSellAllJunkButton:Click()
        end
        if StaticPopup1 and StaticPopup1:IsVisible() and
            StaticPopup1.which == "GENERIC_CONFIRMATION" and StaticPopup1Button1 and
            StaticPopup1Button1:IsEnabled() then
          StaticPopup1Button1:Click()
        end
        for b = 0, 5, 1 do
          for s = 1, C_Container.GetContainerNumSlots(b), 1 do
            local n = C_Container.GetContainerItemLink(b, s)
            if n and select(11, GetItemInfo(n)) > 0 and select(7, GetItemInfo(n)) ~= "Companion Pets" then
              C_Container
        ...
        macroSellUse = DMW.Time
      end
    end
  end
  if DMW.Time < macroUse + 10 then return true end
  if Setting("useVendorMount") then --
    if Player:GetFreeBagSlots() < 40 then
      if Player.Combat then return true end
      if DMW.Time - macroUse >= 4 then
        DMW.Functions.Unlocked.RunMacroText("/cast Traveler's Tundra Mammoth")
        macroUse = DMW.Time
      end
      return true
    end
  ...
  if not Buff.MarkOfTheWild:Exist(Player) then
    if Spell.MarkOfTheWild:Cast(Player) then return true end
  end
  if not Player.Moving and Player.HP < 40 then
  ...
  if Player.SpecID == "Balance" then
    LocalsBalance()
    -- if Spell.WildMushroom:Charges() >= 3 then
    --   local mushroomTarget, tempCount
    --   for _, Unit in ipairs(Enemy30Y) do
    --     local Enemy5t, Enemy5tCount = Unit:GetEnemiesAround(7)
    --     if Enemy5tCount > 0 and (not tempCount or tempCount < Enemy5tCount) then
    --       tempCount = Enemy5tCount
    --       mushroomTarget = Unit
    --     end
    --     -- if Enemy5tCount >= 4 then
    --     --   print(Unit.Name)
    --     --   mushroomTarget = Unit
    --     -- end
    --   end
    --   if mushroomTarget then
    --     if Spell.WildMushroom:Cast(mushroomTarget) then return true end
    --   end
    -- end

    -- if Buff.StarweaversWeft:Exist() and Spell.Starsurge:IsReady() then
    --   for _, Unit in ipairs(Enemy30Y) do
    --     if Spell.Starsurge:Cast(Unit) then return true end
    --   end
    -- end
    if Player.LunarPower >= 50 or Buff.StarweaversWarp:Exist() then
      if Spell.Starfall:Cast(Player) then return true end
    end
    if not Buff.FormMoonkin:Exist(Player) then
      if Spell.FormMoonkin:Cast(Player) then return true end
    end
    -- if Player.Moving and Spell.Starfire:IsReady() and Buff.OwlkinFrenzy:Exist() and not Buff.EclipseLunar:Exist() and
    --     not Buff.EclipseSonar:Exist() then
    --   for _, Unit in ipairs(Enemy30Y) do
    --     if Spell.Starfire:Cast(Unit) then return true end
    --   end
    -- end
    if #DMW.Enemies >= 5 and not Buff.EclipseLunar:Exist() and not Buff.EclipseSonar:Exist() and Spell.CelestialAlignment:IsReady() then
      Spell.CelestialAlignment:Cast()
    end
    if HUD.Pull == 1 then
      table.sort(
        DMW.Attackable,
        function(x, y)
          return x.Distance2D > y.Distance2D
        end
      )

      if Spell.Sunfire:IsReady() and GetRaidTargetIndex("player") then
        for _, Unit in ipairs(DMW.Attackable) do
          -- if Debuff.Moonfire:Refresh(Unit) then

          if Unit.Level <= 60 and Unit.HP > 0 and Unlocked.IsSpellInRange(Spell.Sunfire.SpellName, Unit.GUID) == 1 and not Unit:HasThreat() and Unit:SectorPull() and not Unlocked.UnitIsTapDenied(Unit.GUID) then --not Debuff.Moonfire:Exist(Unit) then --and Unlocked.UnitAffectingCombat(Unit.GUID) then -- not Unit:HasThreat() and not Unlocked.UnitIsTapDenied(Unit.GUID) and
            -- print(Unit.Name, Unit.Health, Unit.Distance, TipsyGuy.UnitTarget(Unit.Pointer))
            if Spell.Sunfire:Cast(Unit, nil, true) then return true end
          end
        end
      end
      if Spell.Sunfire:IsReady() then
        for _, Unit in ipairs(DMW.Attackable) do
          -- if Debuff.Moonfire:Refresh(Unit) then
          if Unit.Level <= 60 and Unit.HP > 0 and Unlocked.IsSpellInRange(Spell.Sunfire.SpellName, Unit.GUID) == 1 and not Unit:HasThreat() and not Unlocked.UnitIsTapDenied(Unit.GUID) then --not Debuff.Moonfire:Exist(Unit) then --and Unlocked.UnitAffectingCombat(Unit.GUID) then -- not Unit:HasThreat() and not Unlocked.UnitIsTapDenied(Unit.GUID) and
            -- print(Unit.Name, Unit.Health, Unit.Distance, TipsyGuy.UnitTarget(Unit.Pointer))
            if Spell.Sunfire:Cast(Unit, nil, true) then return true end
          end
        end
      end
      if Spell.ForceOfNature:IsReady() then
        local castFoN = true
        for _, v in pairs(DMW.Units) do
          if v.ObjectID == 103822 then
            castFoN = false
            break
          end
        end
        if castFoN then
          if Spell.ForceOfNature:CastSpellGround(Player) then return true end
        end
      end
      -- if not Player.Moving and Spell.Wrath:IsReady() and GetSpellCount(5176) > 0 then
      --   for _, Unit in ipairs(Enemy30Y) do
      --     if Unit.TTD > 2 and Unit.Distance < 30 then
      --       if Spell.Wrath:Cast(Unit) then
      --         FaceUnit(Unit)
      --         return true
      --       end
      --     end
    ...
    -- if not Player.Moving and Spell.Wrath:IsReady() and not Buff.EclipseLunar:Exist() then
    --   for _, Unit in ipairs(Enemy30Y) do
    --     if Spell.Wrath:Cast(Unit) then
    --       FaceUnit(Unit)
    --       return true
    ...
    --   end
    -- end
    -- if not Player.Moving and Spell.Wrath:IsReady() then
    --   for _, Unit in ipairs(Enemy30Y) do
    --     if Spell.Wrath:Cast(Unit) then
    --       FaceUnit(Unit)
    --       return true
    ...
    -- end
    for _, Unit in ipairs(Enemy30Y) do
      if Debuff.Sunfire:Refresh(Unit) then
        if Spell.Sunfire:Cast(Unit) then return true end
      end
    end
    ...
    --             Unlocked.RunMacroText("/cast Собиратель трофеев Финдля")
    --             C_Timer.After(1, function () Unlocked.SpellStopCasting() end)
    --             toyTime = DMW.Time + 3
    --             return true
    --         end
    --     end
    ...
    if (Target and Target.ValidEnemy) or (DMW.Player.InstanceID ~= nil and Player.Combat) or not Player.InGroup then
      ...
      if Buff.StarweaversWarp:Exist() and Spell.Starfall:IsReady() then
        if Spell.Starfall:Cast(Player) then return true end
      end
      -- if not Player.Moving and Spell.Starfire:IsReady() and not Buff.EclipseLunar:Exist() and
      --     not Buff.EclipseSonar:Exist() then
      --   for _, Unit in ipairs(Enemy30Y) do
      --     if Spell.Starfire:Cast(Unit) then return true end
      --   end
      -- end
      -- if not Player.Moving and Spell.Wrath:IsReady() and not Buff.EclipseLunar:Exist() and not Buff.EclipseSonar:Exist() then
      --   for _, Unit in ipairs(Enemy30Y) do
      --     if Spell.Wrath:Cast(Unit) then return true end
      --   end
      -- end
      ...
      if Spell.Starsurge:IsReady() and (Buff.EclipseSonar:Exist() or Buff.EclipseLunar:Exist()) then
        for _, Unit in ipairs(Enemy30Y) do
          if Spell.Starsurge:Cast(Unit) then return true end
        end
      end
      if not Player.Moving then
        for _, Unit in ipairs(Enemy30Y) do
          if Debuff.StellarFlare:Refresh(Unit) and Unit.TTD >= 15 then
            if Spell.StellarFlare:Cast(Unit) then return true end
          end
        end
    ...
    if (Target and Target.ValidEnemy) or (DMW.Player.InstanceID ~= nil and Player.Combat) or not Player.InGroup then
      if Target and Target.ValidEnemy then
        ...
        if flyingT and not Debuff.Moonfire:Exist(Target) then
          ...
          if not Buff.Prowl:Exist() and not Player.Combat and (Target.Distance <= 10 or Spell.WildChargeCat:IsReady())
              and Target.Distance <= 25 and Spell.Prowl:IsReady() then
      ...
      if Target and Target.ValidEnemy and Target.Distance <= 5 and (not Buff.Prowl:Exist() or Spell.Prowl:CD() > 5) and
          Target:Facing() then
        if not IsCurrentSpell(Spell.Attack.SpellID) then Unlocked.StartAttack() end
      end
  ...
  elseif Player.SpecID == "Guardian" then
    ...
    --   -- for _, Unit in pairs(DMW.Attackable) do
    --   -- 	-- if Unit.Distance <= 10 and noAoeList[Unit.ObjectID] then
    --   -- 	if Unit.Distance <= 11 and not GrindBot.Grinding:CheckLevel(Unit.Level) and not Unlocked.UnitThreatSituation(DMW.Player.Pointer, Unit.Pointer) then
    --   -- 		-- print(Unit.Name)
    ...
    if (Target and Target.ValidEnemy) or Player.Combat then
      ...
      -- for _, Unit in ipairs(DMW.Attackable) do
      --   -- if Debuff.Moonfire:Refresh(Unit) then
      --   if not Unit:HasThreat() and not Unlocked.UnitIsTapDenied(Unit.GUID) and Unlocked.UnitAffectingCombat(Unit.GUID) then
      --     if Spell.Moonfire:Cast(Unit) then return true end
      --   end
      -- end
      -- if Target and Target.ValidEnemy then
      -- 	local flyingT = Target:Flying()
      -- 	if flyingT and not Debuff.Moonfire:Exist(Target) then
      if Target and Target.ValidEnemy and Spell.Moonfire:IsReady() and not Debuff.Moonfire:Exist(Target) and
          (not Target.Target or Target.Target ~= DMW.Player.Pointer) then
      ...
      --       end
      --     end
      --     if lastHitUnit and DMW.Time - lastHitTime >= 4 then
      ...
      if GrindBot.Combat.MultipullForceCombat then
        ...
        if EnemyMeleeCount >= 5 and Player.HP < 45 and Spell.SurvivalInstincts:IsReady() and
            not Spell.SurvivalInstincts:LastCast() then
    ...
    if (Target and Target.ValidEnemy) or (DMW.Player.InstanceID ~= nil and Player.Combat) or not Player.InGroup then
      ...
      if Form == "Moonkin" then
        if DMW.Friends.LowestHP >= Setting("Critical HP") then
          ...
          Unlocked.RunMacroText("/cancelform")
        end
  ...
  elseif Player.SpecID == "sub10" then
    if not Player.Combat and Player:RunningTime() > 0.4 and (not Spell.WrathSub10.LastCastTime or DMW.Time - Spell.WrathSub10.LastCastTime > 1) then
      if Spell.FormTravel:IsReady() and IsOutdoors() then --and Player.Instance ~= "party" then
        ...
        if Spell.FormCat:IsReady() and not Buff.FormCat:Exist() then
          if Spell.FormCat:Cast() then return true end
        end
      end
    end
    if Target and Target.ObjectID == 166918 then
      if Spell.MoonfireSub10:IsReady() then
        if Debuff.MoonfireSub10:Refresh(Target) then
          if Spell.MoonfireSub10:Cast(Target) then
            return true
          end
        end
        return true
    ...
    if Spell.WrathSub10:IsReady() and not Player.Moving then
      if Target and Target.ValidEnemy then
        if Spell.WrathSub10:Cast(Target) then
          FaceUnit(Target)
          return true
        end
      end
    end
    if Target and Unlocked.UnitAffectingCombat(Target.GUID) and not Target:HasThreat() then
      if Spell.MoonfireSub10:IsReady() then
        if Spell.MoonfireSub10:Cast(Target) then return true end
      end
    end
  end
end
